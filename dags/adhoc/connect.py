...
class ProcessingData():
    ...
    def transform(self,ti,task_ids: str,key :str):
        ...
        if self.db_type=="oracle":
            data = ti.xcom_pull(task_ids=task_ids, key=key)
            print("data",data)
            df1 = pd.read_json(data,convert_axes=False ,convert_dates=False)
            df = df1.copy()
            return df
            # ti.xcom_push('transform_data', df.to_json(orient='records'))
    ...
    def insert_data(self,ti,table_name:str, schema_name:str,task_ids:str, key:str):
        if self.db_type=="postgres":
            ...
            df1 = pd.read_json(data)
            df = df1.copy()
            df["NGAY_CHI"] = pd.to_datetime(df["NGAY_CHI"]).dt.date
            
            # df[list_col_date] = pd.to_datetime(df[list_col_date]).dt.date
    ...
    def insert_data_query(self,query:str):
        hook = OracleHook(self.connection)
        conn = hook.get_conn()            
        cursor = conn.cursor()
