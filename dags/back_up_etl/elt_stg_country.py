...
with DAG(
    ...
    # [END default_args]
    description='ETL DAG Import country',
    ...
    def extract(**kwargs):
        file_path = f'/root/airflow/dags/file/Dsquocgia.csv'
        filename = f'Dsquocgia.csv'
        ti = kwargs['ti']
        if 'csv' in filename:
            df = pd.read_csv(file_path)
        df.drop_duplicates()
        ti.xcom_push('locations_data', df.to_json(orient='records'))
        # df = json.loads(df)
        # return df
    ...
    def transform(**kwargs):
        ...
        ti = kwargs['ti']
        data = ti.xcom_pull(task_ids='extract', key='locations_data')
    ...
    def load(**kwargs):
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_QUOC_GIA', con=engine, schema='STG',
    ...
    def check_data_imported():
        ...
        engine = create_engine(
            f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_QUOC_GIA"'
        df = pd.read_sql(query, con=engine)
