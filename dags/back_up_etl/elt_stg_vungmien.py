...
with DAG(
    ...
    # [END default_args]
    description='ETL DAG Import vung mien',
    ...
    def extract(**kwargs):
        file_path = f'/root/airflow/dags/file/Dsvungmien.csv'
        filename =  f'Dsvungmien.csv'
        ti = kwargs['ti']
        if 'csv' in filename:
            df = pd.read_csv(file_path)
        df.drop_duplicates()
        ti.xcom_push('vungmien', df.to_json(orient='records'))
        # df = json.loads(df)
        # return df
    ...
    def transform(**kwargs):
        ...
        ti = kwargs['ti']
        data = ti.xcom_pull(task_ids='extract', key='vungmien')
    ...
    def load(**kwargs):
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_VUNG_MIEN', con=engine, schema='STG', if_exists='replace',index=False)
        return {"table STG_VUNG_MIEN ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('dwh_postgres')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_VUNG_MIEN"'
        df = pd.read_sql(query, con=engine)
