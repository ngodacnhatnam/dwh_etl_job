...
with DAG(
    ...
    def get_date_import():
        ...
        query = """
        select max(date_import) as max_date from "DWH".date_import_fact
        """
    ...
    def trigger(sdate):
        url = 'http://localhost:8080/api/v1/dags/etl_fact_san_pham/dagRuns'
        headers = {
    ...
    def insert_to_date_import():
        ...
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = """
        insert into "DWH".date_import_fact(date_import, import_at) values(%s, now())
        """
        try:
    ...
    def insert():
        ...
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = """
        INSERT INTO "DWH"."FACT_GIA_SAN_PHAM" 
        SELECT TO_CHAR("NGAY_CAP_NHAT",'YYYYMMDD')::int, "ID_SAN_PHAM", "ID_VUNG_MIEN","GIA_SAN_PHAM" FROM "DWH"."DIM_SAN_PHAM" A 
    ...
    def delete():
        ...
        query = """    
            DELETE FROM "DWH"."FACT_GIA_SAN_PHAM" WHERE "NGAY_DL" =%s
        """
        engine.execute(query,(sdate_string))
    ...
    create_date_import_table_task =  PostgresOperator(
        ...
        sql="""
            CREATE TABLE IF NOT EXISTS "DWH".date_import_fact (
