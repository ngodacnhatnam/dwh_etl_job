...
with DAG(
    'etl_fact_th_sl_nhacc',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='ETL DAG Import fact nhà cung cung cap',
    ...
    def get_date_import():
        ...
        query = """
        select max(date_import) as max_date from "DWH".date_import_fact_th_sl_nhacc
        """
        ...
        for row in rs:
            ...
            if isinstance(date_import, str):
                date_import = datetime.datetime.strptime(
                    date_import, "%Y-%m-%d")
        if not date_import:
            date_import = datetime.datetime.strptime(
                CONSTANT_DATE_IMPORT, "%Y-%m-%d")
    ...
    def trigger(sdate):
        url = 'http://localhost:8080/api/v1/dags/etl_fact_th_sl_nhacc/dagRuns'
        headers = {
    ...
    def insert_to_date_import():
        ...
        engine = create_engine(
            f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = """
        insert into "DWH".date_import_fact_th_sl_nhacc(date_import, import_at) values(%s, now())
        """
        try:
    ...
    def insert():
        ...
        engine = create_engine(
            f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = """
        INSERT INTO "DWH"."FACT_TONG_HOP_SL_NCC" ("NGAY_DL","ID_VUNG_MIEN", "ID_NHOM_SP","ID_HANG_SX" ,"SO_LUONG_NCC"
        )
        SELECT to_char(cc."NGAY_CAP_NHAT",'YYYYMMDD')::INT, vm."ID_VUNG_MIEN"::int AS "ID_VUNG_MIEN",  nsp."ID_NHOM_SP"::INT, hsx."ID_HANG_SX"::INT, count(cc."MA_NCC")::INT
        FROM "DWH"."DIM_NHA_CC" a 
        LEFT JOIN "DWH"."DIM_SAN_PHAM" cc ON cc."MA_NCC" = a."MA_NHA_CC"
        ...
        LEFT JOIN "DWH"."DIM_HANG_SX" hsx ON cc."TEN_HANG_SX" = hsx."TEN_HANG_SX"
				LEFT JOIN "DWH"."DIM_VUNG_MIEN" vm ON a."TEN_TINH_THANH" = vm."TEN_TINH_THANH"
        WHERE cc."NGAY_CAP_NHAT" = TO_DATE(%s,'YYYY-MM-DD')
        GROUP BY cc."NGAY_CAP_NHAT", "ID_VUNG_MIEN",  nsp."ID_NHOM_SP", hsx."ID_HANG_SX"
        """
    ...
    def delete():
        ...
        query = """    
            DELETE FROM "DWH"."FACT_TONG_HOP_SL_NCC" WHERE TO_CHAR(TO_DATE('20200710', 'YYYYMMDD'), 'YYYY-MM-DD')= %s
        """
        engine.execute(query, (sdate_string))
    ...
    create_date_import_table_task = PostgresOperator(
        ...
        sql="""
            CREATE TABLE IF NOT EXISTS "DWH".date_import_fact_th_sl_nhacc (
