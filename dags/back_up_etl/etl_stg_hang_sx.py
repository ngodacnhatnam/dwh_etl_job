...
with DAG(
    ...
    # [END default_args]
    description='ETL DAG Import HANG SX',
    ...
    def extract(**kwargs):
        ...
        query = "select id as MA_HANG_SX, brand_name as TEN_HANG_SX from brand"
    ...
    def load(**kwargs):
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_HANG_SX', con=engine, schema='STG', if_exists='replace', index=False)
        return {"table STG_HANG_SX ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('dwh_postgres')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_HANG_SX"'
        df = pd.read_sql(query, con=engine)
