...
with DAG(
    'etl_stg_khaithac_khoangsan',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='ETL DAG Import khai thac khaong san',
    schedule_interval=None,
    start_date=pendulum.datetime(2022, 1, 1, tz="UTC"),
    catchup=False,
    tags=['etl','stg','import','daklak'],
) as dag:
    # [END instantiate_dag]
    # [START documentation]
    dag.doc_md = __doc__
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name='khaithachkhoangsan', header=0, names=['MA_KHOANG_SAN','TEN_KHOANG_SAN','TONG_TRU_LUONG','TL_DA_KHAI_THAC','NAM_BAO_CAO'])
        ti = kwargs['ti']
        df.drop_duplicates()
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE "STG"."STG_TINH_TRANG_KT_KHOANG_SAN";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_TINH_TRANG_KT_KHOANG_SAN', con=engine, schema='STG', if_exists='append',index=False)
        return {"table STG_TINH_TRANG_KT_KHOANG_SAN ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('daklak_dwh')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_TINH_TRANG_KT_KHOANG_SAN"'
