...
with DAG(
    'etl_stg_pl_khoangsan',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='ETL DAG Import phân loại khoáng sản',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path,  sheet_name='khoangsan', header=0, names=['TEN_KHOANG_SAN','MA_KHOANG_SAN'])
        ti = kwargs['ti']
        df.drop_duplicates()
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE "STG"."STG_LOAI_KHOANG_SAN";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_LOAI_KHOANG_SAN', con=engine, schema='STG', if_exists='append',index=False)
        return {"table STG_LOAI_KHOANG_SAN ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('daklak_dwh')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_LOAI_KHOANG_SAN"'
