...
with DAG(
    ...
    # [END default_args]
    description='ETL DAG Import phân loại đất',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name='loai_dat', header=0, names=['MA_LOAI_DAT_CAP1','TEN_LOAI_DAT_CAP1','MA_LOAI_DAT_CAP2','TEN_LOAI_DAT_CAP2','MA_LOAI_DAT_CAP3','TEN_LOAI_DAT_CAP3','MA_MAP_LOAI_DAT'])
        ti = kwargs['ti']
        df.drop_duplicates()
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE "STG"."STG_PHAN_LOAI_DAT";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_PHAN_LOAI_DAT', con=engine, schema='STG', if_exists='append',index=False)
        return {"table STG_PHAN_LOAI_DAT ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('daklak_dwh')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_PHAN_LOAI_DAT"'
