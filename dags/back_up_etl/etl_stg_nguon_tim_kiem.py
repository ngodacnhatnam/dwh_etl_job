...
with DAG(
    'etl_stg_nguon_tim_kiem',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='ETL DAG Import nguồn tìm kiếm',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name=f'CAC_NGUON_TIM_KIEM', header=0, names=['LOAI_HINH', 'SL_ONLINE', 'SL_OFLINE'] )
        ti = kwargs['ti']
        df.drop_duplicates()
        ti.xcom_push('nguon_tim_kiem', df.to_json(orient='records'))
    ...
    def transform(**kwargs):
        ...
        ti = kwargs['ti']
        data = ti.xcom_pull(task_ids='extract', key='nguon_tim_kiem')
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE  "STG"."STG_NGUON_TIM_KIEM";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_NGUON_TIM_KIEM', con=engine, schema='STG', if_exists='replace',index=False)
        return {"table STG_NGUON_TIM_KIEM ": "Data imported successful"}

    def check_data_imported():
        hook = PostgresHook(postgres_conn_id="dwh_postgres")
        conn = hook.get_conn()
        query = 'select * from "STG"."STG_NGUON_TIM_KIEM"'
