...
with DAG(
    'etl_stg_phuongthuc_thanhtoan',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='ETL DAG Import phương thức thanh toán',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name=f'PHUONG_THUOC_TT', header=0, names=['HINH_THUC_TT','TY_LE'])
        ti = kwargs['ti']
        df.drop_duplicates()
        ti.xcom_push('phuongthuc_tt', df.to_json(orient='records'))
    ...
    def transform(**kwargs):
        ...
        ti = kwargs['ti']
        data = ti.xcom_pull(task_ids='extract', key='phuongthuc_tt')
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE  "STG"."STG_PHUONG_THUC_TT";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_PHUONG_THUC_TT', con=engine, schema='STG', if_exists='replace',index=False)
        return {"table STG_PHUONG_THUC_TT ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('dwh_postgres')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_PHUONG_THUC_TT"'
