...
with DAG(
    'etl_stg_SoLieuDLTinh',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='Lưu thông tin về số liệu du lịch của tỉnh',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name='Số liệu du lịch của một tỉnh', header=0, names=['PHAN_LOAI','SO_LUONG','NAM'])
        ti = kwargs['ti']
        df.drop_duplicates()
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE "STG"."STG_SO_LIEU_DL_TINH";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_SO_LIEU_DL_TINH', con=engine, schema='STG', if_exists='append',index=False)
        return {"table STG_SO_LIEU_DL_TINH ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('dwh_etl')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_SO_LIEU_DL_TINH"'
