...
with DAG(
    'etl_stg_tinhtrang_sd_dat',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='ETL DAG Import tình trạng sử dụng đất',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name='dat', header=0, names=['MA_LOAI_DAT','TEN_LOAI_DAT','MA_DON_VI_SD','TEN_DON_VI_SD','DIEN_TICH','NAM_BAO_CAO'])
        ti = kwargs['ti']
        df.drop_duplicates()
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE "STG"."STG_TINH_TRANG_SU_DUNG_DAT";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_TINH_TRANG_SU_DUNG_DAT', con=engine, schema='STG', if_exists='append',index=False)
        return {"table STG_TINH_TRANG_SU_DUNG_DAT ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('daklak_dwh')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_TINH_TRANG_SU_DUNG_DAT"'
