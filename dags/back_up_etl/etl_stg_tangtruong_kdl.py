...
with DAG(
    'etl_stg_tangtruong_kdl',
    # [START default_args]
    # These args will get passed on to each operator
    # You can override them on a per-task basis during operator initialization
    default_args={'retries': 2},
    # [END default_args]
    description='ETL DAG Import tăng trưởng khách du lịch',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name=f'Tẳng trưởng KDL quốc tế', header=0, names=['TINH_THANH','TANG_TRUONG_NAM_TRUOC','TANG_TRUONG_NAM_NAY','BAO_CAO'])
        ti = kwargs['ti']
        df.drop_duplicates()
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE  "STG"."STG_TANG_TRUONG_KHACH_DL";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_TANG_TRUONG_KHACH_DL', con=engine, schema='STG', if_exists='replace',index=False)
        return {"table STG_TANG_TRUONG_KHACH_DL ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('dwh_postgres')
        engine = create_engine(f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_TANG_TRUONG_KHACH_DL"'
