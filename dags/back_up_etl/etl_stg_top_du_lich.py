...
with DAG(
    ...
    # [END default_args]
    description='ETL DAG Import top du lịch',
    ...
    def extract(**kwargs):
        ...
        if 'xlsx' in filename:
            df = pd.read_excel(file_path, sheet_name=f'TOP_DU_LICH', header=0, names=[
                               'DIA_DIEM', 'SO_LUOT', 'DOANH_THU'])
        ti = kwargs['ti']
        df.drop_duplicates()
    ...
    def load(**kwargs):
        ...
        cursor = conn.cursor()
        delete_sql_query = """
        TRUNCATE TABLE  "STG"."STG_TOP_DU_LICH";
        """
        ...
        df1 = pd.read_json(data)
        df = df1.copy()
        df.to_sql('STG_TOP_DU_LICH', con=engine, schema='STG',
                  if_exists='replace', index=False)
        return {"table STG_TOP_DU_LICH ": "Data imported successful"}

    def check_data_imported():
        conn = BaseHook.get_connection('dwh_postgres')
        engine = create_engine(
            f'postgresql://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        query = 'select * from "STG"."STG_TOP_DU_LICH"'
