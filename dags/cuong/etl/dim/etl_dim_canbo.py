...
import pendulum
from cuong.general_function.connect import Connection, MYDAG
from cuong.general_function.scd_type_2 import SCD_TYPE_2
...
mypath = '/home/airflow/airflow/dags/cuong/declare/dim/canbo.json'
myDag = MYDAG(mypath)


class DIM_CAN_BO(SCD_TYPE_2):
    ...
    def get_source_df(self):
        query = """SELECT a.MA_CAN_BO, a.TEN_CAN_BO, b.ID_PHONG_BAN, c.ID_CHUC_VU  
                FROM "DWH_STG"."STG_THONG_TIN_CAN_BO" a 
                LEFT JOIN "DWH_DIM"."DIM_PHONG_BAN" b 
                ON trim(UPPER(a.PHONG_BAN))=trim(UPPER(b.TEN_PHONG_BAN))
                LEFT JOIN "DWH_DIM"."DIM_CHUC_VU" c 
                on trim(UPPER(a.CHUC_VU))=trim(UPPER(c.TEN_CHUC_VU))
                """
        engine = self.get_engine(self.config["source"]["CONNECTION"])
...
with DAG(
    ...
    def load(**kwargs):
        ...
        A."TEN_CAN_BO" != TRIM(B."TEN_CAN_BO") OR A."CHUC_VU" != TRIM(B."CHUC_VU") OR A."PHONG_BAN" != TRIM(B."PHONG_BAN") OR A."NGAY_HL_BG" != to_date(B."NGAY_NHAN_CHUC",'dd/mm/YYYY') 
        );
