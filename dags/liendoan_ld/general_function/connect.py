...
class MYDAG_DIM():
    ...
    def __init__(self,my_path):
        ...
        self.ALL_COL = self.VARIABLE["ALL_COL"]
        self.TABLE_KEY=self.VARIABLE["TABLE_KEY"]
        self.DATE_COL = self.VARIABLE["DATE_COL"]
...
class Connection():
    ...
    def postgre_conn(self, query: str):
    ...
    def oracle_conn(self,query:str):
    ...
    def execute_oracle_query(self,query:str):
    ...
    def execute_oracle_query_df(self,query:str):
        conn = OracleHook.get_connection(self.connection)
        engine = create_engine(
                f'oracle+cx_oracle://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        df = pd.read_sql(query, con=engine)
