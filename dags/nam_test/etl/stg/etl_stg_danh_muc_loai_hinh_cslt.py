...
from nam_test.general_function.connect import Connection, MYDAG
from nam_test.general_function.processing_data import ProcessingData

mypath = '/home/airflow/airflow/dags/nam_test/declare/stg/ma_loai_hinh_cslt.json'
myDag = MYDAG(mypath)
...
with DAG(
    ...
    def extract(**kwargs):
        path = '/home/airflow/airflow/dags/file/ma_loai_hinh_cslt.csv'
        df = pd.read_csv(path)
        conn = BaseHook.get_connection('dwh_oracle')
        engine = create_engine(f'oracle+cx_oracle://{conn.login}:{conn.password}@{conn.host}:{conn.port}/{conn.schema}')
        ti = kwargs['ti']
        df.to_sql('STG_MA_LOAI_HINH_CSLT_TEST_NAMNGO', con=engine, if_exists='append', index=False)
